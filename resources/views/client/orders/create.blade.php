...
<div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
    ...
    <div>
        ...
        <p style="font-size:.82rem;color:var(--text-sub);">Fill in the shipment details below</p>
    </div>
</div>
...
<div class="grid-2" style="align-items:start;">
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        {{-- Shipment Info card --}}
        <div class="card">
            <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:16px;">Shipment Details</div>

            <div class="form-group">
                <label class="form-label" for="order_description">Shipment Contents / Description</label>
                <textarea id="order_description" name="order_description" class="form-textarea" placeholder="E.g. Electronics — 2 items">{{ old('order_description') }}</textarea>
            </div>
            ...
            <div class="form-group">
                ...
                <div style="display:flex;gap:12px;">
                    <label style="flex:1;display:flex;align-items:center;gap:9px;background:var(--in-bg);border:1px solid var(--in-bdr);border-radius:9px;padding:12px 14px;cursor:pointer;" id="codLabel">
                        ...
                        <span style="font-size:.88rem;font-weight:600;">COD</span>
                        <span style="font-size:.74rem;color:var(--text-dim);">Cash on delivery</span>
                    </label>
                    <label style="flex:1;display:flex;align-items:center;gap:9px;background:var(--in-bg);border:1px solid var(--in-bdr);border-radius:9px;padding:12px 14px;cursor:pointer;" id="prepaidLabel">
                        ...
                        <span style="font-size:.88rem;font-weight:600;">Paid</span>
                        <span style="font-size:.74rem;color:var(--text-dim);">Already paid</span>
                    </label>
                </div>
            </div>

            <div id="codPriceGroup" class="form-group" style="{{ old('payment_type','cod') !== 'cod' ? 'display:none' : '' }}">
                <label class="form-label" for="order_price">COD Order Price (JD) *</label>
            ...
            <div class="form-group">
                <label style="display:flex;align-items:center;gap:10px;cursor:pointer;">
                    ...
                    <span style="font-size:.88rem;font-weight:600;">Delivery Charges On Customer</span>
                </label>
                <div class="form-hint">Enable if the customer pays the delivery fee separately.</div>
            </div>

            <div id="customerFeeGroup" class="form-group" style="{{ old('delivery_on_customer') ? '' : 'display:none' }}">
                <label class="form-label" for="delivery_customer_amount">Customer Delivery Fee (JD) *</label>
        ...
        {{-- Notes card --}}
        <div class="card">
            <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:16px;">Instructions</div>
            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label" for="notes">Special Delivery Instructions / Notes</label>
                <textarea id="notes" name="notes" class="form-textarea" placeholder="Any special instructions for the delivery driver…">{{ old('notes') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div style="display:flex;flex-direction:column;gap:16px;">
        {{-- Receiver card --}}
        <div class="card">
            <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:16px;">Receiver Information</div>

            <div class="form-group">
                <label class="form-label" for="receiver_name">Receiver Name *</label>
                <input id="receiver_name" name="receiver_name" type="text" class="form-input {{ $errors->has('receiver_name') ? 'has-error' : '' }}" placeholder="Full name" value="{{ old('receiver_name') }}">
            ...
            <div class="form-group">
                ...
                <input id="receiver_phone" name="receiver_phone" type="tel" class="form-input {{ $errors->has('receiver_phone') ? 'has-error' : '' }}" placeholder="07xxxxxxxx" value="{{ old('receiver_phone') }}">
            ...
            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label" for="address_text">Address Details *</label>
                <textarea id="address_text" name="address_text" class="form-textarea {{ $errors->has('address_text') ? 'has-error' : '' }}" placeholder="Street, building, floor, landmarks…" style="min-height:72px;">{{ old('address_text') }}</textarea>
