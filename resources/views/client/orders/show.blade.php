...
<div class="page-hd">
    <div class="page-hd-left" style="display:flex;align-items:center;gap:12px;">
        ...
        <div>
            ...
            <p>{{ __('Created :date', ['date' => $order->created_at->format('d M Y, H:i')]) }}</p>
        </div>
    </div>
    <div class="page-hd-right" style="display:flex;align-items:center;gap:8px;">
        ...
        <a href="{{ route('client.orders.pdf', ['ids' => $order->id]) }}" target="_blank" class="btn-secondary" style="padding:7px 14px;font-size:.82rem;display:inline-flex;align-items:center;gap:6px;">
            <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1M4 10V6a2 2 0 012-2h6l6 6v4"/></svg>
            {{ __('Download PDF') }}
        </a>
...
{{-- QR Code --}}
<div class="info-card" style="margin-bottom:16px; display:flex; align-items:center; justify-content:space-between; gap:24px; flex-wrap:wrap;">
    <div>
        <div class="info-card-title" style="margin-bottom:8px;">{{ __('Order QR Code') }}</div>
        <p style="font-size:.82rem;color:var(--text-sub);max-width:380px;line-height:1.6;">
            {{ __('Scan to retrieve order') }} <strong style="color:var(--text);">#{{ $order->order_number }}</strong>.<br>
            {{ __('Present this at pickup or show the driver for quick verification.') }}
        </p>
    </div>
    <div style="display:flex;flex-direction:column;align-items:center;gap:10px;">
        ...
        <span style="font-size:.78rem;font-weight:700;color:var(--text-sub);letter-spacing:.08em;font-family:monospace;">#{{ $order->order_number }}</span>
    </div>
</div>
...
<div class="info-grid">
    {{-- Tracking History --}}
    <div class="info-card">
        <div class="info-card-title">{{ __('Tracking History') }}</div>
        @if($order->trackingLogs->isEmpty())
            <div style="text-align:center;padding:24px 0;color:var(--text-dim);font-size:.85rem;">{{ __('No tracking events yet.') }}</div>
        @else
        <div class="timeline">
            ...
            <div class="timeline-item {{ $loop->first ? 'active' : 'completed' }} status-{{ $log->to_status }}">
                ...
                <div class="timeline-title">{{ __('Status:') }}
                ...
                @if($log->user)
                <div class="timeline-user">{{ __('Updated by:') }} {{ $log->user->name }}</div>
                @endif
            </div>
            @endforeach
        </div>
    ...
    <div class="info-card">
        <div class="info-card-title">{{ __('Proof of Delivery') }}</div>
        <div class="info-rows" style="gap:16px;">
            @if($order->status === 'delivered')
                <div>
                    <div style="font-size:.72rem;font-weight:700;color:rgba(255,255,255,.45);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px;">{{ __('Customer Signature') }}</div>
                    ...
                    @else
                        <span style="font-size:.82rem;color:var(--text-dim);font-style:italic;">{{ __('No digital signature captured') }}</span>
                    @endif
                </div>
                <div>
                    <div style="font-size:.72rem;font-weight:700;color:rgba(255,255,255,.45);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px;">{{ __('Proof of Delivery Photo') }}</div>
                    ...
                    @else
                        <span style="font-size:.82rem;color:var(--text-dim);font-style:italic;">{{ __('No proof photo uploaded') }}</span>
                    @endif
                </div>
                <div>
                    ...
                    @if($order->national_id_attachment_path)
                        ...
                        @else
                            <a href="{{ asset('storage/' . $order->national_id_attachment_path) }}" target="_blank"
                               style="display:inline-flex;align-items:center;gap:6px;padding:7px 14px;background:rgba(220,38,38,.08);border:1px solid rgba(220,38,38,.25);border-radius:8px;color:var(--red-lt);font-size:.85rem;text-decoration:none;">
                        ...
                        @endif
                    @else
                        <span style="font-size:.82rem;color:var(--text-dim);font-style:italic;">{{ __('No national ID uploaded') }}</span>
                    @endif
                </div>
            @elseif($order->status === 'rejected')
                <div>
                    <div style="font-size:.72rem;font-weight:700;color:rgba(220,38,38,.6);letter-spacing:.08em;text-transform:uppercase;margin-bottom:8px;">{{ __('Rejection Reason') }}</div>
                    <strong style="color:#f87171;">{{ $order->rejectionReason?->reason ?? __('Not specified') }}</strong>
                </div>
            @else
                <div style="text-align:center;padding:24px 0;color:var(--text-dim);font-size:.85rem;">
                    {{ __('Proof of delivery will appear here once the order is delivered.') }}
                </div>
            @endif
        </div>
    </div>
...
<div id="deleteModal" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.65);backdrop-filter:blur(4px);z-index:500;align-items:center;justify-content:center;">
    <div style="background:#0c1230;border:1px solid var(--bdr);border-radius:16px;padding:28px 30px;max-width:400px;width:90%;animation:fu .25s both;">
        ...
        <p style="font-size:.86rem;color:var(--text-sub);">{{ __('Delete order :number? This cannot be undone.', ['number' => $order->order_number]) }}</p>
