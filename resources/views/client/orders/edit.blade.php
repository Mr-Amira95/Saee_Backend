...
<div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
    <a href="{{ route('client.orders.show', $order) }}" class="btn-secondary" style="padding:7px 14px;font-size:.82rem;">{{ __('← Back') }}</a>
    <div>
        ...
        <p style="font-size:.82rem;color:var(--text-sub);">{{ __('Changes are only allowed while the order is pending.') }}</p>
    </div>
</div>
...
<div class="form-wrap">
    <form action="{{ route('client.orders.update', $order) }}" method="POST" id="orderForm">
            ...
            <div class="form-grid-2" style="margin-top:18px;border-top:1px solid var(--bdr);padding-top:18px;">
                <div class="form-group" style="flex-direction:row;align-items:center;gap:10px;">
                    <label class="toggle-switch">
                        <input type="checkbox" name="delivery_on_customer" id="delivery_on_customer" value="1"
        ...
        <div class="form-actions">
            <a href="{{ route('client.orders.show', $order) }}" class="btn-secondary">{{ __('Cancel') }}</a>
...
<script>
    ...
    const preselectedAreaId  = {{ old('area_id', $receiver?->area_id ?? 'null') }};
    ...
    class SearchableSelect {
        ...
        _renderDropdown(filter) {
            ...
            const matches = q ? this.options.filter(o => o.text.toLowerCase().includes(q)) : this.options;
    ...
    function populateAreas(selectedCityId, selectedAreaId) {
        areaSelect.innerHTML = `<option value="">${selectAreaLabel}</option>`;
        ...
        if (selectedCityId) {
            ...
            if (city && city.areas) {
                city.areas.forEach(area => {
                    ...
                    if (area.id == selectedAreaId) option.selected = true;
    ...
    citySelect.addEventListener('change', function () {
        const areaId = areaSS ? null : preselectedAreaId;
        populateAreas(this.value, areaId);
    });

    // Pre-select city and areas on page load
    if (preselectedCityId) {
        citySelect.value = preselectedCityId;
        const cityOpt = citySelect.options[citySelect.selectedIndex];
        if (cityOpt && cityOpt.value) citySS.input.value = cityOpt.text;
        populateAreas(preselectedCityId, preselectedAreaId);
    }
