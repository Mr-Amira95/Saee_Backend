...
<div style="display:flex;align-items:center;gap:10px;margin-bottom:20px;">
    ...
    <div>
        <h1 style="font-size:1.3rem;font-weight:800;">{{ __('Banking Details') }}</h1>
        <p style="font-size:.82rem;color:var(--text-sub);">{{ __('Bank account information for settlements and payouts') }}</p>
    </div>
</div>
...
<div class="card" style="margin-bottom:16px;">
    <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:18px;">{{ __('Bank Account') }}</div>
    ...
    <div class="form-group">
        ...
        <input id="account_name" name="account_name" type="text" class="form-input {{ $errors->has('account_name') ? 'has-error' : '' }}"
               placeholder="{{ __('As it appears on the account') }}" value="{{ old('account_name', $bankDetail?->account_name) }}" required>
...
<div class="card" style="margin-bottom:16px;">
    <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:18px;">{{ __('CliQ Details') }}</div>

    <div class="form-group">
        <label class="form-label">{{ __('CliQ Type') }}</label>
        <div style="display:flex;gap:12px;">
            <label style="display:flex;align-items:center;gap:9px;background:var(--in-bg);border:1px solid var(--in-bdr);border-radius:9px;padding:10px 14px;cursor:pointer;">
                ...
                <span style="font-size:.87rem;font-weight:600;">{{ __('Alias') }}</span>
            </label>
            <label style="display:flex;align-items:center;gap:9px;background:var(--in-bg);border:1px solid var(--in-bdr);border-radius:9px;padding:10px 14px;cursor:pointer;">
                ...
                <span style="font-size:.87rem;font-weight:600;">{{ __('Phone Number') }}</span>
            </label>
        </div>
    </div>
...
<div class="card" style="margin-bottom:20px;">
    <div style="font-size:.76rem;font-weight:700;color:var(--text-dim);text-transform:uppercase;letter-spacing:.1em;margin-bottom:14px;">{{ __('Notes') }}</div>
    <div class="form-group" style="margin-bottom:0;">
        <textarea name="notes" class="form-textarea" placeholder="{{ __('Any additional banking notes or instructions…') }}">{{ old('notes', $bankDetail?->notes) }}</textarea>
    </div>
</div>
