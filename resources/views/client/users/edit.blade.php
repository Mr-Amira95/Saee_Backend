    ...
    <div class="card" style="margin-bottom:20px; max-width:650px;">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;">
            <div class="form-group">
                ...
                <input id="phone" type="text" name="phone" value="{{ old('phone', $employee->user->phone) }}" required class="form-input {{ $errors->has('phone') ? 'has-error' : '' }}" placeholder="{{ __('e.g. 0791234567') }}">
                @error('phone') <div class="form-error">{{ $message }}</div> @enderror
            </div>

            <div class="form-group">
                <label class="form-label" for="email">{{ __('Email Address *') }}</label>
                <input id="email" type="email" name="email" value="{{ old('email', $employee->user->email) }}" required class="form-input {{ $errors->has('email') ? 'has-error' : '' }}" placeholder="{{ __('e.g. employee@company.com') }}">
