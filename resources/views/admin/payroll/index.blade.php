...
@section('title', __('Driver Payroll'))
@section('page-title', __('Driver Payroll'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Payroll') }}</span>
@endsection

@section('content')
    <div class="page-hd">
        <div class="page-hd-left">
            <h1>{{ __('Driver Payroll') }}</h1>
            <p>{{ __('Track and process monthly salary payments to drivers.') }}</p>
        </div>
    </div>
    ...
    <div class="filter-bar">
        <form action="{{ route('admin.payroll.index') }}" method="GET" class="filter-form">
            ...
            <a href="{{ route('admin.payroll.index') }}" class="btn-secondary" style="padding: 8px 16px;">{{ __('Reset') }}</a>
        </form>

        <div style="margin-left: auto;">
            <div style="font-size:.8rem;color:var(--text-dim);">
                {{ __('Select a driver below to create a new payroll entry →') }}
            </div>
        </div>
    </div>
    ...
    <div class="table-card">
                    ...
                    @forelse($payments as $p)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="cell-sub">{{ __('to') }} {{ $p->period_end->format('d M Y') }}</div>
                            </td>
                            ...
                            <td>
                                <div class="act-btns" style="justify-content: center;">
                                    <a href="{{ route('admin.payroll.show', $p) }}" class="act-btn act-view" title="{{ __('View') }}">
        ...
        @if($payments->hasPages())
            <div class="pagination-wrap">
                <div class="pag-info">{{ __('Showing') }} {{ $payments->firstItem() }}-{{ $payments->lastItem() }} {{ __('of') }} {{ $payments->total() }}</div>
