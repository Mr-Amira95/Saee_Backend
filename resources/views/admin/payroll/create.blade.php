...
@section('title', __('New Payroll —') . ' ' . $driver->user->name)
@section('page-title', __('New Payroll Entry'))

@section('breadcrumb')
    <span class="sep">/</span>
    <a href="{{ route('admin.payroll.index') }}">{{ __('Payroll') }}</a>
    <span class="sep">/</span>
    <span class="current">{{ __('New —') }} {{ $driver->user->name }}</span>
@endsection

@section('content')
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        <a href="{{ route('admin.payroll.index') }}" class="btn-secondary">{{ __('← Back') }}</a>
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('New Payroll Entry —') }} {{ $driver->user->name }}</h1>
    </div>
    ...
    <div style="background:rgba(234,179,8,.08);border:1px solid rgba(234,179,8,.35);border-radius:10px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:center;gap:12px;">
        ...
        <div style="font-size:.88rem;color:#fde047;">
            {{ __('A payroll record already exists for this driver covering') }}
            ...
            <a href="{{ route('admin.payroll.show', $existingThisMonth) }}" style="color:#fde047;text-decoration:underline;margin-left:6px;">{{ __('View it →') }}</a>
        </div>
    </div>
    @endif
    ...
    <form method="POST" action="{{ route('admin.payroll.store', $driver) }}">
        ...
        <div class="form-section">
            <div class="form-section-title">{{ __('Pay Period') }}</div>
        ...
        <div class="form-section">
            <div class="form-section-title">{{ __('Salary Breakdown') }}</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label class="form-label">{{ __('Basic Salary') }} <span class="req">*</span></label>
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('Car & Gasoline Allowance') }} <span class="req">*</span></label>
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('Daily Order Threshold') }}</label>
                    ...
                    <span style="font-size:.75rem;color:var(--text-dim);margin-top:4px;display:block;">
                        {{ __('Orders per day above this count earn a bonus — used to auto-fill the field below') }}
                    </span>
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Bonus Per Extra Order') }}</label>
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('Extra Orders Above Threshold') }}</label>
                    <input type="number" name="extra_orders_count" id="extra-orders-count" class="form-input @error('extra_orders_count') err @enderror"
                           ...
                           oninput="recalc()">
                    <span style="font-size:.75rem;color:var(--text-dim);margin-top:4px;display:block;">
                        {{ __('Auto-calculated from daily threshold above — or enter manually') }}
                    </span>
                ...
                <div class="form-group" style="display:flex;flex-direction:column;justify-content:flex-end;">
                    <div style="background:rgba(34,197,94,.08);border:1px solid rgba(34,197,94,.25);border-radius:10px;padding:14px 16px;">
                        <div style="font-size:.72rem;color:var(--text-dim);text-transform:uppercase;letter-spacing:.06em;margin-bottom:4px;">{{ __('Net Amount') }}</div>
        ...
        <div class="form-section">
            <div class="form-section-title">{{ __('Payment Details') }}</div>
            <div class="form-grid-2">
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="reference_number" class="form-input"
                           value="{{ old('reference_number') }}" placeholder="{{ __('e.g. TXN-1234') }}">
                </div>
                <div class="form-group" style="grid-column: span 2;">
                    <label class="form-label">{{ __('Notes') }} <span class="opt">{{ __('(optional)') }}</span></label>
                    <textarea name="notes" class="form-input" rows="3"
                              placeholder="{{ __('Any additional notes...') }}" style="height:auto;">{{ old('notes') }}</textarea>
                </div>
            </div>
        </div>

        <div class="form-actions">
            <a href="{{ route('admin.payroll.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
            @if(auth()->user()->hasAdminAction('finances.driver_payroll'))
            <button type="submit" class="btn-primary">{{ __('Create Draft') }}</button>
            @endif
        </div>
    </form>
@endsection
