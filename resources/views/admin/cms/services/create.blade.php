...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Add Logistical Service') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.services.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="title[en]" class="form-input @error('title.en') err @enderror"
                           value="{{ old('title.en') }}" placeholder="e.g. Next-Day Delivery" required>
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <textarea name="subtitle[en]" class="form-input @error('subtitle.en') err @enderror" rows="5"
                              placeholder="Explain what the service covers, rates, SLA, etc..." style="height: auto;">{{ old('subtitle.en') }}</textarea>
                    @error('subtitle.en')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Subtitle (Arabic)') }}</label>
                    <textarea name="subtitle[ar]" dir="rtl" class="form-input @error('subtitle.ar') err @enderror" rows="5" style="height: auto;">{{ old('subtitle.ar') }}</textarea>
        ...
        <div class="form-actions" style="margin-top:20px;">
            ...
            <button type="submit" class="btn-primary">{{ __('Save Service') }}</button>
        </div>
    </form>
</div>
@endsection
