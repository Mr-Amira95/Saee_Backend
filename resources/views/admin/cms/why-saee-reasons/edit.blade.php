...
@section('breadcrumb')
    <span class="sep">/</span>
    <a href="{{ route('admin.cms.why-saee-reasons.index') }}">{{ __('Reasons') }}</a>
    <span class="sep">/</span>
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        <a href="{{ route('admin.cms.why-saee-reasons.index') }}" class="btn-secondary">&#8592; {{ __('Back') }}</a>
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Edit Reason') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.why-saee-reasons.update', $reason) }}" enctype="multipart/form-data">
        ...
        <div class="form-section">
            <div class="form-section-title">{{ __('Reason Details') }}</div>
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="title[en]" class="form-input @error('title.en') err @enderror"
                           value="{{ old('title.en', $reason->title['en'] ?? '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title[ar]" dir="rtl" class="form-input @error('title.ar') err @enderror"
                           value="{{ old('title.ar', $reason->title['ar'] ?? '') }}" required>
            ...
            <div class="form-grid-2" style="margin-top:15px;">
                <div class="form-group">
                    ...
                    <textarea name="subtitle[en]" class="form-input @error('subtitle.en') err @enderror" rows="3" style="height:auto;">{{ old('subtitle.en', $reason->subtitle['en'] ?? '') }}</textarea>
                    @error('subtitle.en')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Subtitle (Arabic)') }}</label>
                    <textarea name="subtitle[ar]" dir="rtl" class="form-input @error('subtitle.ar') err @enderror" rows="3" style="height:auto;">{{ old('subtitle.ar', $reason->subtitle['ar'] ?? '') }}</textarea>
            ...
            <div class="form-group" style="margin-top:15px;">
                ...
                @if($reason->icon_path)
                    <div style="margin-bottom: 10px;">
                        <img src="{{ $reason->icon_path }}" alt="{{ __('Current icon') }}" style="width: 42px; height: 42px; object-fit: contain; border-radius: 8px; border: 1px solid var(--bdr); background: rgba(255,255,255,.05); padding: 6px;">
                    </div>
                @endif
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="number" name="sort_order" class="form-input"
                           value="{{ old('sort_order', $reason->sort_order) }}" min="0" required>
                </div>

                <div class="form-group">
                    <label class="form-label">{{ __('Status') }}</label>
                    <select name="status" class="form-input" style="background: var(--in-bg); color: var(--text);">
                        <option value="active" {{ old('status', $reason->status) === 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                        <option value="inactive" {{ old('status', $reason->status) === 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions" style="margin-top:20px;">
            <a href="{{ route('admin.cms.why-saee-reasons.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
            <button type="submit" class="btn-primary">{{ __('Update Reason') }}</button>
        </div>
    </form>
</div>
@endsection
