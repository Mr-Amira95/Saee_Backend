...
@section('title', __('Hero Section'))
@section('page-title', __('Hero Section'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('Hero Section') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage Hero Section') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.hero.update') }}" enctype="multipart/form-data">
        ...
        <div class="form-section" style="margin-top:20px;">
            <div class="form-section-title">{{ __('Hero Image') }}</div>
            <div style="background: rgba(255,255,255,.02); border: 1px solid var(--bdr); border-radius: 10px; padding: 20px; margin-bottom: 15px;">
                @if($hero->image_path)
                    <div style="margin-bottom: 15px;">
                        ...
                        <img src="{{ $hero->image_path }}" alt="{{ __('Hero image') }}" style="max-width: 320px; height: 180px; object-fit: cover; border-radius: 8px; border: 1px solid var(--bdr);">
                    </div>
                @endif
                <div class="form-grid-2">
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="image_path" class="form-input @error('image_path') err @enderror"
                               value="{{ old('image_path', $hero->image_path) }}" placeholder="{{ __('e.g. https://example.com/hero.jpg') }}">
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save Hero Section') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('Hero Stats') }}</h2>
        <a href="{{ route('admin.cms.hero-stats.index') }}" class="btn-secondary">{{ __('Manage Stats') }} &rarr;</a>
    </div>
</div>
@endsection
