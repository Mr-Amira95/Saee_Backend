...
@section('breadcrumb')
    <span class="sep">/</span>
    <a href="{{ route('admin.cms.about-values.index') }}">About Values</a>
    <span class="sep">/</span>
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        <a href="{{ route('admin.cms.about-values.index') }}" class="btn-secondary">&#8592; Back</a>
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">Add Value</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.about-values.store') }}">
        @csrf

        <div class="form-section">
            <div class="form-section-title">Value Text</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label class="form-label">Text (English) <span class="req">*</span></label>
                    <input type="text" name="text[en]" class="form-input @error('text.en') err @enderror"
                           value="{{ old('text.en') }}" placeholder="e.g. Speed without compromise" required>
                    @error('text.en')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Text (Arabic) <span class="req">*</span></label>
                    <input type="text" name="text[ar]" dir="rtl" class="form-input @error('text.ar') err @enderror"
                           value="{{ old('text.ar') }}" required>
        ...
        <div class="form-actions" style="margin-top:20px;">
            <a href="{{ route('admin.cms.about-values.index') }}" class="btn-secondary">Cancel</a>
            <button type="submit" class="btn-primary">Save Value</button>
        </div>
    </form>
</div>
@endsection
