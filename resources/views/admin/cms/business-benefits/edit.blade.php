...
@section('title', __('Edit Benefit'))
@section('page-title', __('Edit Benefit'))

@section('breadcrumb')
    <span class="sep">/</span>
    <a href="{{ route('admin.cms.business-benefits.index') }}">{{ __('Business Benefits') }}</a>
    <span class="sep">/</span>
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        <a href="{{ route('admin.cms.business-benefits.index') }}" class="btn-secondary">{{ __('← Back') }}</a>
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Edit Benefit') }}</h1>
    </div>
        ...
        <div class="form-section" style="margin-top:20px;">
                ...
                <div class="form-group">
                    ...
                    @if($benefit->icon_path)
                        <div style="margin-bottom: 10px;">
                            <img src="{{ $benefit->icon_path }}" alt="{{ __('Current icon') }}" style="width: 42px; height: 42px; object-fit: contain; border-radius: 8px; border: 1px solid var(--bdr); background: rgba(255,255,255,.05); padding: 6px;">
                        </div>
                    @endif
            ...
            <div class="form-group" style="margin-top:15px;">
                ...
                <select name="status" class="form-input" style="background: var(--in-bg); color: var(--text);">
                    <option value="active" {{ old('status', $benefit->status) === 'active' ? 'selected' : '' }}>{{ __('Active') }}</option>
                    <option value="inactive" {{ old('status', $benefit->status) === 'inactive' ? 'selected' : '' }}>{{ __('Inactive') }}</option>
                </select>
            </div>
        </div>

        <div class="form-actions" style="margin-top:20px;">
            <a href="{{ route('admin.cms.business-benefits.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
            <button type="submit" class="btn-primary">{{ __('Update Benefit') }}</button>
        </div>
    </form>
</div>
@endsection
