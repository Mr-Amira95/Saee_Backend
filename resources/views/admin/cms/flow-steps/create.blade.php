...
@section('title', __('Add Flow Step'))
@section('page-title', __('Add Flow Step'))
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Add Flow Step') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.flow-steps.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-section" style="margin-top:20px;">
            ...
            <div style="background: rgba(255,255,255,.02); border: 1px solid var(--bdr); border-radius: 10px; padding: 20px; margin-bottom: 15px;">
                <div class="form-grid-2">
                    <div class="form-group">
                        <label class="form-label">{{ __('Upload Image File') }}</label>
                        <input type="file" name="image_file" class="form-input @error('image_file') err @enderror"
                               accept="image/*" style="height: auto; padding: 8px;">
                        <span style="font-size: .75rem; color: var(--text-dim); margin-top: 4px;">{{ __('Max size: 4MB.') }}</span>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="image_path" class="form-input @error('image_path') err @enderror"
                               value="{{ old('image_path') }}" placeholder="{{ __('e.g. https://example.com/step.jpg') }}">
        ...
        <div class="form-actions" style="margin-top:20px;">
            ...
            <button type="submit" class="btn-primary">{{ __('Save Step') }}</button>
        </div>
    </form>
</div>
@endsection
