...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">Edit Capability</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.showcase-capabilities.update', $capability) }}">
            ...
            <div class="form-grid-2">
                ...
                <div class="form-group">
                    ...
                    <select name="status" class="form-input" style="background: var(--in-bg); color: var(--text);">
                        <option value="active" {{ old('status', $capability->status) === 'active' ? 'selected' : '' }}>Active</option>
                        <option value="inactive" {{ old('status', $capability->status) === 'inactive' ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
        </div>
        ...
        <div class="form-actions" style="margin-top:20px;">
            ...
            <button type="submit" class="btn-primary">Update Capability</button>
        </div>
    </form>
</div>
@endsection
