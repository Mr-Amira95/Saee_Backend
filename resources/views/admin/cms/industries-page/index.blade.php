...
@section('title', __('Industries Section'))
@section('page-title', __('Industries Section'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('Industries Section') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage Industries Section') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.industries-page.update') }}">
        @csrf
        @method('PUT')
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="badge[en]" class="form-input @error('badge.en') err @enderror"
                           value="{{ old('badge.en', $industrySection->badge['en'] ?? '') }}" placeholder="e.g. Industries We Serve">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="badge[ar]" dir="rtl" class="form-input @error('badge.ar') err @enderror"
                           value="{{ old('badge.ar', $industrySection->badge['ar'] ?? '') }}" placeholder="مثال: الصناعات التي نخدمها">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="title[en]" class="form-input @error('title.en') err @enderror"
                           value="{{ old('title.en', $industrySection->title['en'] ?? '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="title[ar]" dir="rtl" class="form-input @error('title.ar') err @enderror"
                           value="{{ old('title.ar', $industrySection->title['ar'] ?? '') }}" required>
        ...
        <div class="form-section" style="margin-top:20px;">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <textarea name="subtitle[en]" class="form-input @error('subtitle.en') err @enderror" rows="3" style="height:auto;">{{ old('subtitle.en', $industrySection->subtitle['en'] ?? '') }}</textarea>
                    @error('subtitle.en')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Subtitle (Arabic)') }}</label>
                    <textarea name="subtitle[ar]" dir="rtl" class="form-input @error('subtitle.ar') err @enderror" rows="3" style="height:auto;">{{ old('subtitle.ar', $industrySection->subtitle['ar'] ?? '') }}</textarea>
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save Industries Section') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('Industries List') }}</h2>
        <a href="{{ route('admin.cms.industries.index') }}" class="btn-secondary">{{ __('Manage Industries List') }} &rarr;</a>
    </div>
</div>
@endsection
