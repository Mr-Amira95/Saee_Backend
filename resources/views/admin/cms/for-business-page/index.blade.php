...
@section('title', __('For Businesses Page'))
@section('page-title', __('For Businesses Page'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('For Businesses Page') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage For Businesses Page') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.for-business-page.update') }}">
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save For Businesses Page') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('Benefits') }}</h2>
        <a href="{{ route('admin.cms.business-benefits.index') }}" class="btn-secondary">{{ __('Manage Benefits') }} &rarr;</a>
    </div>
</div>
@endsection
