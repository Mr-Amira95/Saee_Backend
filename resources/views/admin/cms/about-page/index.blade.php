...
@section('page-title', __('About Page'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('About Page') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage About Page') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.about-page.update') }}" enctype="multipart/form-data">
        ...
        <div class="form-section" style="margin-top:20px;">
            <div class="form-section-title">{{ __('About Image') }}</div>
            <div style="background: rgba(255,255,255,.02); border: 1px solid var(--bdr); border-radius: 10px; padding: 20px; margin-bottom: 15px;">
                @if($page->image_path)
                    <div style="margin-bottom: 15px;">
                        ...
                        <img src="{{ $page->image_path }}" alt="{{ __('About image') }}" style="max-width: 320px; height: 180px; object-fit: cover; border-radius: 8px; border: 1px solid var(--bdr);">
                    </div>
                @endif
                <div class="form-grid-2">
                    <div class="form-group">
        ...
        <div class="form-section" style="margin-top:20px;">
            <div class="form-section-title">{{ __('Mission') }}</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label class="form-label">{{ __('Mission (English)') }} <span class="req">*</span></label>
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('Mission (Arabic)') }} <span class="req">*</span></label>
        ...
        <div class="form-section" style="margin-top:20px;">
            <div class="form-section-title">{{ __('Vision') }}</div>
            <div class="form-grid-2">
                <div class="form-group">
                    <label class="form-label">{{ __('Vision (English)') }} <span class="req">*</span></label>
                ...
                <div class="form-group">
                    <label class="form-label">{{ __('Vision (Arabic)') }} <span class="req">*</span></label>
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save About Page') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('Values') }}</h2>
        <a href="{{ route('admin.cms.about-values.index') }}" class="btn-secondary">{{ __('Manage Values →') }}</a>
    </div>
</div>
@endsection
