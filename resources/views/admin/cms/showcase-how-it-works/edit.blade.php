...
@section('breadcrumb')
    <span class="sep">/</span>
    <a href="{{ route('admin.cms.showcase-how-it-works.index') }}">{{ __('How It Works') }}</a>
    <span class="sep">/</span>
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        <a href="{{ route('admin.cms.showcase-how-it-works.index') }}" class="btn-secondary">{{ __('← Back') }}</a>
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Edit Step') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.showcase-how-it-works.update', $step) }}" enctype="multipart/form-data">
        @csrf
        @method('PUT')

        <div class="form-section">
            <div class="form-section-title">{{ __('Step Details') }}</div>

            <div class="form-group">
                <label class="form-label">{{ __('Icon (SVG file)') }}</label>
                @if($step->icon_path)
                    <div style="margin-bottom: 10px;">
                        <img src="{{ $step->icon_path }}" alt="{{ __('Current icon') }}" style="width: 42px; height: 42px; object-fit: contain; border-radius: 8px; border: 1px solid var(--bdr); background: rgba(255,255,255,.05); padding: 6px;">
                    </div>
                @endif
        ...
        <div class="form-actions" style="margin-top:20px;">
            <a href="{{ route('admin.cms.showcase-how-it-works.index') }}" class="btn-secondary">{{ __('Cancel') }}</a>
