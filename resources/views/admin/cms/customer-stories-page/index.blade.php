...
@section('title', __('Customer Stories'))
@section('page-title', __('Customer Stories'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('Customer Stories') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage Customer Stories Section') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.customer-stories-page.update') }}">
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save Customer Stories Section') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('Testimonials') }}</h2>
        <a href="{{ route('admin.cms.customer-testimonials.index') }}" class="btn-secondary">{{ __('Manage Testimonials →') }}</a>
    </div>
</div>
@endsection
