...
@section('title', __("Why Sa'ee Section"))
@section('page-title', __("Why Sa'ee Section"))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __("Why Sa'ee Section") }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __("Manage Why Sa'ee Section") }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.why-saee-page.update') }}">
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __("Save Why Sa'ee Section") }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __("Why Sa'ee Reasons") }}</h2>
        <a href="{{ route('admin.cms.why-saee-reasons.index') }}" class="btn-secondary">{{ __('Manage Reasons →') }}</a>
    </div>
</div>
@endsection
