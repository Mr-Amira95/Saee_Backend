...
@section('title', __('Add Screenshot'))
@section('page-title', __('Add Screenshot'))
...
<div>
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:24px;">
        ...
        <h1 style="font-size:1.2rem;font-weight:700;margin:0;">{{ __('Add Screenshot') }}</h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.cms.showcase-screenshots.store') }}" enctype="multipart/form-data">
        ...
        <div class="form-section">
            ...
            <div class="form-group">
                ...
                <select name="category" class="form-input" style="background: var(--in-bg); color: var(--text);">
                    <option value="application" {{ old('category', $category) === 'application' ? 'selected' : '' }}>{{ __('Application') }}</option>
                    <option value="portal" {{ old('category', $category) === 'portal' ? 'selected' : '' }}>{{ __('Portal') }}</option>
                </select>
            </div>

            <div class="form-group" style="margin-top:15px;">
                <label class="form-label">{{ __('Screenshot Image') }} <span class="req">*</span></label>
                ...
                <span style="font-size: .75rem; color: var(--text-dim); margin-top: 4px;">{{ __('Upload a PNG/JPG screenshot. Max size: 4MB.') }}</span>
        ...
        <div class="form-actions" style="margin-top:20px;">
            ...
            <button type="submit" class="btn-primary">{{ __('Save Screenshot') }}</button>
        </div>
    </form>
</div>
@endsection
