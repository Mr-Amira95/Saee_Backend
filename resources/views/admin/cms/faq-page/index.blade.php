...
@section('page-title', __('FAQ Page'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Website CMS') }}</span>
    <span class="sep">/</span>
    <span class="current">{{ __('FAQ Page') }}</span>
@endsection

@section('content')
<div>
    <h1 style="font-size:1.2rem;font-weight:700;margin-bottom:20px;">{{ __('Manage FAQ Page') }}</h1>
    ...
    <form method="POST" action="{{ route('admin.cms.faq-page.update') }}">
        @csrf
        @method('PUT')
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="page_badge[en]" class="form-input @error('page_badge.en') err @enderror"
                           value="{{ old('page_badge.en', $faqPage->page_badge['en'] ?? '') }}" placeholder="e.g. Got Questions?">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="page_badge[ar]" dir="rtl" class="form-input @error('page_badge.ar') err @enderror"
                           value="{{ old('page_badge.ar', $faqPage->page_badge['ar'] ?? '') }}">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <input type="text" name="page_title[en]" class="form-input @error('page_title.en') err @enderror"
                           value="{{ old('page_title.en', $faqPage->page_title['en'] ?? '') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="page_title[ar]" dir="rtl" class="form-input @error('page_title.ar') err @enderror"
                           value="{{ old('page_title.ar', $faqPage->page_title['ar'] ?? '') }}" required>
        ...
        <div class="form-section" style="margin-top:20px;">
            ...
            <div class="form-grid-2">
                <div class="form-group">
                    ...
                    <textarea name="page_subtitle[en]" class="form-input @error('page_subtitle.en') err @enderror" rows="3" style="height:auto;">{{ old('page_subtitle.en', $faqPage->page_subtitle['en'] ?? '') }}</textarea>
                    @error('page_subtitle.en')<span class="form-error">{{ $message }}</span>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">{{ __('Subtitle (Arabic)') }}</label>
                    <textarea name="page_subtitle[ar]" dir="rtl" class="form-input @error('page_subtitle.ar') err @enderror" rows="3" style="height:auto;">{{ old('page_subtitle.ar', $faqPage->page_subtitle['ar'] ?? '') }}</textarea>
        ...
        <div class="form-actions" style="margin-top:20px;">
            <button type="submit" class="btn-primary">{{ __('Save FAQ Page') }}</button>
        </div>
    </form>

    <div style="margin-top:32px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:10px;">
        <h2 style="font-size:1.05rem;font-weight:700;margin:0;">{{ __('FAQ List') }}</h2>
        <a href="{{ route('admin.cms.faqs.index') }}" class="btn-secondary">{{ __('Manage FAQ List →') }}</a>
    </div>
</div>
@endsection
