...
@section('page-title', __('Client Invoices'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Invoices') }}</span>
@endsection

@section('content')
    <div class="page-hd">
        <div class="page-hd-left">
            <h1>{{ __('Client Invoices') }}</h1>
            <p>{{ __('View payout histories, gross COD collections, delivery charge debits, and printable receipts.') }}</p>
        </div>
    </div>
    ...
    <div class="filter-bar">
        <form action="{{ route('admin.financials.invoices') }}" method="GET" class="filter-form">
            <div class="filter-search-wrap">
                ...
                <input type="text" name="search" value="{{ request('search') }}" class="filter-input" placeholder="{{ __('Search by INV number or merchant name...') }}">
            </div>
            ...
            <a href="{{ route('admin.financials.invoices') }}" class="btn-secondary" style="padding: 8px 16px;">{{ __('Reset') }}</a>
        </form>
    </div>
    ...
    <div class="table-card">
                    ...
                    @forelse($invoices as $inv)
                        <tr>
                            ...
                            <td>
                                ...
                                <div class="cell-sub">{{ __('ID:') }} {{ $inv->clientProfile->id }}</div>
                            </td>
                            <td>
                            ...
                            <td style="text-align: center;">
                                <span class="badge badge-pv">{{ $inv->total_orders }} {{ __('orders') }}</span>
                            </td>
                            ...
                            <td>
                                <div class="act-btns" style="justify-content: center;">
                                    <a href="{{ route('admin.financials.invoices.show', $inv) }}" class="act-btn act-view" title="{{ __('View & Print Invoice') }}">
                    ...
                    @empty
                        <tr>
                            <td colspan="8" style="text-align: center; color: var(--text-dim); padding: 40px;">
                                {{ __('No invoices have been created/processed yet.') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if($invoices->hasPages())
            <div class="pagination-wrap">
                <div class="pag-info">{{ __('Showing') }} {{ $invoices->firstItem() }}-{{ $invoices->lastItem() }} {{ __('of') }} {{ $invoices->total() }} {{ __('invoices') }}</div>
                <div class="pag-links">
                    {{ $invoices->links() }}
                </div>
            </div>
