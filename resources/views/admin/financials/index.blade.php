...
@section('page-title', __('Financial Settlements'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Finance Dashboard') }}</span>
@endsection
...
@section('content')
    ...
    <div class="page-hd">
        <div class="page-hd-left">
            <h1>{{ __('Financial Settlements') }}</h1>
            <p>{{ __('Audit cash collections, settle driver cash accounts, and run client COD payouts.') }}</p>
        </div>
    </div>
    ...
    <div style="margin-top: 18px;">
        <div style="display: flex; border-bottom: 2px solid var(--bdr); gap: 4px;">
            <button id="tab-drivers-btn" onclick="switchTab('drivers')"
                ...
                <div style="text-align: left;">
                    <div>{{ __('Total Cash Held by Drivers') }}</div>
            ...
            <button id="tab-clients-btn" onclick="switchTab('clients')"
                ...
                <div style="text-align: left;">
                    <div>{{ __('Total Pending Client Payouts') }}</div>
        ...
        <div id="tab-drivers" style="margin-top: 18px;">
            <div class="table-card">
                <div style="padding: 16px; border-bottom: 1px solid var(--bdr); display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="font-size: 0.9rem; font-weight: 700; color: var(--text-sub); text-transform: uppercase; letter-spacing: 0.08em;">{{ __('Driver Cash Collections') }}</h3>
                    <span style="font-size: 0.72rem; color: var(--text-dim);">{{ __('Settle collected cash to office') }}</span>
                </div>
                        ...
                        <tbody>
                            @forelse($driverBalances as $data)
                                <tr>
                                    ...
                                    <td>
                                        <span class="badge badge-pending">{{ $data['pending_orders_count'] }} {{ __('orders') }}</span>
                                    </td>
                                    <td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" style="text-align: center; color: var(--text-dim); padding: 30px;">
                                        {{ __('All drivers have fully settled their cash collections.') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
        ...
        <div id="tab-clients" style="margin-top: 18px; display: none;">
            <div class="table-card">
                <div style="padding: 16px; border-bottom: 1px solid var(--bdr); display: flex; justify-content: space-between; align-items: center;">
                    <h3 style="font-size: 0.9rem; font-weight: 700; color: var(--text-sub); text-transform: uppercase; letter-spacing: 0.08em;">{{ __('Client Payout Accounts') }}</h3>
                    <span style="font-size: 0.72rem; color: var(--text-dim);">{{ __('Transfer COD collections to Clients') }}</span>
                </div>
                <div class="table-wrap">
                    <table>
                        <thead>
                            <tr>
                                ...
                                <th>{{ __('Pending COD') }}</th>
                                <th>{{ __('Net Balance Due') }}</th>
                                <th style="width: 100px; text-align: center;">{{ __('Actions') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse($clientBalances as $data)
                                <tr>
                                    <td>
                                        ...
                                        <div class="cell-sub">{{ __('ID:') }} {{ $data['client']->id }}</div>
                                    </td>
                                    <td>
                                        <span class="badge badge-info">{{ $data['pending_payout_count'] }} {{ __('orders') }}</span>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="act-btns" style="justify-content: center;">
                                            ...
                                            @else
                                                <span class="cell-sub" style="font-style: italic;">{{ __('No pending payout') }}</span>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" style="text-align: center; color: var(--text-dim); padding: 30px;">
                                        {{ __('No outstanding payouts due to clients.') }}
                                    </td>
                                </tr>
                            @endforelse
                        </tbody>
