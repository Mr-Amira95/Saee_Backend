...
@section('page-title', __('Reporting Center'))

@section('breadcrumb')
    <span class="sep">/</span>
    <span class="current">{{ __('Reports Center') }}</span>
@endsection
...
@section('content')
    ...
    <div class="page-hd">
        <div class="page-hd-left">
            <h1>{{ __('Reporting Center') }}</h1>
            <p>{{ __('Live metrics and operational audits.') }}</p>
        </div>
        <div>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(190px, 1fr)); gap: 14px; margin-bottom: 20px;">
        ...
        <div class="metric" style="--m-color:#60a5fa;--m-icon-bg:rgba(59,130,246,.12);animation-delay:.05s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:var(--text-dim);font-weight:600;">{{ __('All Time') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#4ade80;--m-icon-bg:rgba(34,197,94,.12);animation-delay:.1s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:#4ade80;font-weight:700;">{{ __('✓ Done') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#60a5fa;--m-icon-bg:rgba(59,130,246,.12);animation-delay:.15s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:#60a5fa;font-weight:700;">{{ __('Live') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#fbbf24;--m-icon-bg:rgba(245,158,11,.12);animation-delay:.2s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:#fbbf24;font-weight:700;">{{ __('Queued') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#f87171;--m-icon-bg:rgba(239,68,68,.12);animation-delay:.25s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:#f87171;font-weight:700;">{{ __('Failed') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#c084fc;--m-icon-bg:rgba(168,85,247,.12);animation-delay:.3s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:var(--text-dim);font-weight:600;">{{ __('Active') }}</span>
            </div>
        ...
        <div class="metric" style="--m-color:#34d399;--m-icon-bg:rgba(16,185,129,.12);animation-delay:.35s">
            <div class="metric-head">
                ...
                <span style="font-size:.68rem;color:var(--text-dim);font-weight:600;">{{ __('Onboarded') }}</span>
            </div>
    ...
    <div class="reports-main-grid">
        ...
        <div class="chart-panel" style="display: flex; flex-direction: column;">
            <div class="panel-heading">
                <span>{{ __('7-Day Orders Volume Trend') }}</span>
        ...
        {{-- Status Distribution --}}
        <div class="chart-panel" style="display: flex; flex-direction: column;">
            <div class="panel-heading">{{ __('Status Distribution') }}</div>

            @php
                $total = max(array_sum($statusCounts), 1);
                $dist  = [
                    ['key' => 'delivered',  'label' => __('Delivered'),    'color' => '#4ade80'],
                    ['key' => 'pending',    'label' => __('Pending'),      'color' => '#fbbf24'],
                    ['key' => 'picked_up',  'label' => __('In Transit'),   'color' => '#60a5fa'],
                    ['key' => 'rejected',   'label' => __('Rejected'),     'color' => '#f87171'],
                    ['key' => 'returned',   'label' => __('Returned'),     'color' => '#fb923c'],
                    ['key' => 'cancelled',  'label' => __('Cancelled'),    'color' => '#94a3b8'],
                ];
            @endphp
