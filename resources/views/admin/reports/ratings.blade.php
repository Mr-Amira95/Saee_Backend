...
@section('title', __('Customer Ratings Log'))
@section('page-title', __('Ratings Log'))
...
@section('breadcrumb')
    ...
    <span class="sep">/</span>
    @endif
    <span class="current">{{ __('Ratings Log') }}</span>
@endsection
...
@section('content')
    ...
    <div class="page-hd">
        <div class="page-hd-left">
            <h1>{{ __('Customer Ratings Log') }}</h1>
            <p>{{ __('Monitor customer feedback and ratings left for drivers upon order deliveries.') }}</p>
        </div>
    ...
    <div class="mini-stats" style="margin-bottom:20px;">
        <div class="mini-stat">
            ...
            <div>
                ...
                <div class="mini-stat-lbl">{{ __('Global Average Rating') }}</div>
            </div>
        </div>
        <div class="mini-stat">
            ...
            <div>
                ...
                <div class="mini-stat-lbl">{{ __('Total Customer Reviews') }}</div>
            </div>
        </div>
    </div>
    ...
    <form method="GET" action="{{ route('admin.reports.ratings') }}" class="ratings-filter-bar">
        <div class="form-group">
            <label class="form-label" for="driverFilter">{{ __('Filter by Driver') }}</label>
        ...
        <div class="form-group">
            <label class="form-label" for="ratingFilter">{{ __('Filter by Rating') }}</label>
            <select name="rating" id="ratingFilter" class="form-select" style="padding: 8px 30px 8px 11px;">
                <option value="">{{ __('All Stars') }}</option>
                @for($i=5; $i>=1; $i--)
                    <option value="{{ $i }}" {{ request('rating') == $i ? 'selected' : '' }}>{{ $i }} {{ __('Stars') }}</option>
                @endfor
            </select>
        </div>

        <div class="form-group">
            <label class="form-label" for="dateFrom">{{ __('Date From') }}</label>
            <input type="date" name="date_from" id="dateFrom" class="form-input" value="{{ request('date_from') }}" style="padding: 7px 11px;">
        </div>

        <div class="form-group">
            <label class="form-label" for="dateTo">{{ __('Date To') }}</label>
        ...
        <div class="filter-btn-group">
            ...
            @if(request()->anyFilled(['driver', 'rating', 'date_from', 'date_to']))
                <a href="{{ route('admin.reports.ratings') }}" class="btn-secondary" style="padding:8px 16px;">{{ __('Clear') }}</a>
            @endif
        </div>
    </form>
    ...
    <div class="table-card" style="animation: fu .45s .1s both;">
        @if($ratings->isEmpty())
            <div class="empty-state">
                ...
                <h3>{{ __('No Ratings Found') }}</h3>
                <p>{{ __('No customer reviews match the selected filter criteria.') }}</p>
            </div>
        @else
            <div class="table-wrap">
                <table>
                    <thead>
                        <tr>
                            <th style="width: 120px;">{{ __('Order Number') }}</th>
                            <th>{{ __('Driver') }}</th>
                            <th style="width: 130px;">{{ __('Rating') }}</th>
                        ...
                        @foreach($ratings as $r)
                            <tr>
                                <td>
                                    ...
                                    @else
                                        <span style="color:var(--text-dim);">{{ __('N/A') }}</span>
                                    @endif
                                </td>
                                <td>
                                    @if($r->driver)
                                    ...
                                    @else
                                        <span style="color:var(--text-dim);">{{ __('Unknown Driver') }}</span>
                                    @endif
                                </td>
                                <td>
                                ...
                                <td>
                                    ...
                                    @else
                                        <span style="color:var(--text-dim);font-style:italic;">{{ __('No text review') }}</span>
                                    @endif
                                </td>
                                <td>
            ...
            <div class="pagination-wrap">
                <div class="pag-info">
                    {{ __('Showing') }} {{ $ratings->firstItem() ?? 0 }} {{ __('to') }} {{ $ratings->lastItem() ?? 0 }} {{ __('of') }} {{ $ratings->total() }} {{ __('ratings') }}
                </div>
                <div class="pag-links">
                    {{ $ratings->links() }}
                </div>
