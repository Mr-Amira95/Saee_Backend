...
@section('title', __('Edit Client –') . ' ' . $client->company_name)
...
<form method="POST" action="{{ route('admin.clients.update', $client) }}" enctype="multipart/form-data" novalidate>
    ...
    <div class="form-section" style="position:relative;z-index:3;">
        <div class="form-section-title">{{ __('Account Owner (Master User)') }}</div>
        <div class="form-grid-2">
            ...
            <div class="form-group">
                <label class="form-label" for="phone">{{ __('Personal Phone') }} <span class="opt">({{ __('required for WhatsApp') }})</span></label>
    ...
    <div class="form-section" style="position:relative;z-index:2;">
        ...
        <div class="form-grid-2">
            ...
            <div class="form-group">
                ...
                <input class="form-input @error('commercial_register_number') is-error @enderror" id="commercial_register_number" type="text" inputmode="numeric" pattern="[0-9]*" name="commercial_register_number" value="{{ old('commercial_register_number', $client->commercial_register_number) }}" placeholder="e.g. 1234567" title="{{ __('Numbers only') }}">
            ...
            <div class="form-group">
                ...
                <input class="form-input @error('vat_number') is-error @enderror" id="vat_number" type="text" inputmode="numeric" pattern="[0-9]*" name="vat_number" value="{{ old('vat_number', $client->vat_number) }}" placeholder="e.g. 1234567" title="{{ __('Numbers only') }}">
    ...
    <div class="form-section" style="position:relative;z-index:1;">
        ...
        <div class="form-grid-2">
            <div class="form-group" style="grid-column:1/-1;">
                ...
                <input class="form-input" id="address_line1" type="text" name="address_line1" value="{{ old('address_line1', $client->address_line1) }}" placeholder="{{ __('Street, Building, Floor…') }}">
            </div>
        ...
        <div class="form-grid-2">
            ...
            <div class="form-group">
                ...
                <input class="form-input" id="account_name" type="text" name="account_name"
                       value="{{ old('account_name', $bd->account_name ?? '') }}" placeholder="{{ __('As it appears on the account') }}">
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <input class="form-input" id="account_number" type="text" name="account_number"
                       value="{{ old('account_number', $bd->account_number ?? '') }}" placeholder="{{ __('Bank account number') }}"
            ...
            <div class="form-group">
                ...
                <div style="display:flex;gap:8px;">
                    <select name="cliq_alias_type" id="cliq_alias_type" class="form-select" style="width:150px;flex-shrink:0;">
                        ...
                        <option value="alias" {{ old('cliq_alias_type', $bd->cliq_alias_type ?? '') === 'alias' ? 'selected' : '' }}>{{ __('Alias') }}</option>
                        <option value="phone" {{ old('cliq_alias_type', $bd->cliq_alias_type ?? '') === 'phone' ? 'selected' : '' }}>{{ __('Phone') }}</option>
                    </select>
            ...
            <div class="form-group" style="grid-column:1/-1;">
                ...
                <textarea class="form-input" id="bank_notes" name="bank_notes" rows="2"
                          ...
                          placeholder="{{ __('Any additional banking notes or instructions…') }}">{{ old('bank_notes', $bd->notes ?? '') }}</textarea>
            </div>
        </div>
    </div>
    ...
    <div class="form-section">
        ...
        <div class="logo-upload-zone" id="logoZone">
            ...
            <div id="logoPreviewWrap" style="pointer-events:none;">
                <img id="logoPreviewImg" class="logo-preview-img" src="{{ Storage::disk('public')->url($client->logo_path) }}" alt="{{ __('Logo preview') }}">
                <div style="font-size:.78rem;color:var(--text-sub);margin-top:6px;" id="logoFileName">{{ __('Current Logo') }}</div>
            </div>
            @else
    ...
    <div class="form-section">
        ...
        <div style="margin-bottom: 16px;">
            <div style="font-size: .8rem; font-weight: 700; color: var(--text-dim); text-transform: uppercase; margin-bottom: 8px;">{{ __('Existing Attachments') }}</div>
            ...
            <div class="exist-att" id="exist-att-{{ $att->id }}">
                ...
                <div class="exist-att-del">
                    ...
                    <label for="del-att-{{ $att->id }}">{{ __('Delete') }}</label>
                </div>
            </div>
            @endforeach
        </div>
...
const i18nClientEdit = {
    ...
    validDateFormat: @json(__(i18nClientEdit.validDateFormat)),
    cliqPhoneInvalid: @json(__(i18nClientEdit.cliqPhoneInvalid)),
    cliqAliasInvalid: @json(__(i18nClientEdit.cliqAliasInvalid)),
    deliveryPriceMin: @json(__('The Delivery price of :city must be at least 0')),
    fullNameInvalid: @json(__(i18nClientEdit.fullNameInvalid)),
    usernameInvalid: @json(__(i18nClientEdit.usernameInvalid)),
    emailInvalid: @json(__(i18nClientEdit.emailInvalid)),
    phoneInvalid: @json(__(i18nClientEdit.phoneInvalid)),
    bankNameInvalid: @json(__(i18nClientEdit.bankNameInvalid)),
    accountNameInvalid: @json(__(i18nClientEdit.accountNameInvalid)),
    ibanInvalid: @json(__(i18nClientEdit.ibanInvalid)),
    swiftInvalid: @json(__(i18nClientEdit.swiftInvalid)),
    accountNumberInvalid: @json(__(i18nClientEdit.accountNumberInvalid)),
    fullNameRequired: @json(__(i18nClientEdit.fullNameRequired)),
    usernameRequired: @json(__(i18nClientEdit.usernameRequired)),
    companyNameRequired: @json(__(i18nClientEdit.companyNameRequired)),
};
...
function initSearchableSelect(btnId, ddId, listId, hiddenId, opts) {
    ...
    function applyFilter() {
        ...
        if (q && visible === 0) {
            ...
            emptyEl.textContent = i18nClientEdit.noResultsFor.replace(':query', search.value);
...
function loadAreas(cityId) {
    if (!cityId) {
        areaBtn._reset(i18nClientEdit.selectGovernorateFirst);
        areaBtn._setEnabled(false);
        return;
    }
    areaBtn._reset(i18nClientEdit.loading);
    areaBtn._setEnabled(false);
    fetch(AREAS_URL + '?city_id=' + cityId)
        ...
        .then(function(areas) {
            ...
            areaBtn._populate(items, i18nClientEdit.selectArea, OLD_AREA_ID);
...
document.getElementById('logoInput').addEventListener('change', function() {
    ...
    if (this.files[0].size > 2 * 1024 * 1024) {
        showWarningToast(i18nClientEdit.logoTooLarge);
        this.value = '';
        return;
    }
...
function addAttachRow() {
    ...
    for (let r of rows) {
        ...
        if (labelInput && !labelInput.value.trim()) {
            showWarningToast(i18nClientEdit.fillAttachmentLabel);
            labelInput.focus();
            return;
        }
        if (fileInput && !fileInput.files.length) {
            showWarningToast(i18nClientEdit.chooseAttachmentFile);
            return;
        }
    }
    ...
    row.innerHTML =
        '<div><input type="text" name="attachment_labels[' + i + ']" class="form-input" ' +
             'placeholder="' + i18nClientEdit.attachmentLabelPlaceholder + '" style="height:38px;"></div>' +
    ...
    row.querySelector('input[type="file"]').addEventListener('change', function() {
        if (this.files[0] && this.files[0].size > 10 * 1024 * 1024) {
            showWarningToast(i18nClientEdit.attachmentTooLarge);
            this.value = '';
        }
    });
}
...
(function() {
    ...
    if (expiryEl) {
        expiryEl.addEventListener('input', function() {
            ...
            if (out.length === 10 && !isValidExpiryDate(out)) {
                showFieldError(expiryEl, i18nClientEdit.validDateFormat);
            }
        });
    }
    ...
    function validateCliq() {
        ...
        if (type === 'phone' && !isValidCliqPhone(val)) {
            showFieldError(cliqIdEl, i18nClientEdit.cliqPhoneInvalid);
        } else if (type === 'alias' && !isValidCliqAlias(val)) {
            showFieldError(cliqIdEl, i18nClientEdit.cliqAliasInvalid);
        }
    }
    ...
    form.querySelectorAll('.delivery-price-input').forEach(function(el) {
        el.addEventListener('input', function() {
            ...
            if (el.value !== '' && parseFloat(el.value) < 0) {
                ...
                showFieldError(el, i18nClientEdit.deliveryPriceMin.replace(':city', el.dataset.city));
            }
        });
    });
    ...
    wireLiveValidation('name', isValidName, i18nClientEdit.fullNameInvalid);
    wireLiveValidation('username', isValidUsername, i18nClientEdit.usernameInvalid);
    wireLiveValidation('email', isEmail, i18nClientEdit.emailInvalid);
    wireLiveValidation('phone', isValidPhone, i18nClientEdit.phoneInvalid);
    wireLiveValidation('bank_name', isValidEnglishName, i18nClientEdit.bankNameInvalid);
    wireLiveValidation('account_name', isValidEnglishName, i18nClientEdit.accountNameInvalid);
    wireLiveValidation('iban', isValidIban, i18nClientEdit.ibanInvalid);
    wireLiveValidation('swift_code', isValidSwift, i18nClientEdit.swiftInvalid);
    wireLiveValidation('account_number', isValidAccountNumber, i18nClientEdit.accountNumberInvalid);
