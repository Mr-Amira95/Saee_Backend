...
<head>
    ...
    <style>
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .icon-wrap { width: 56px; height: 56px; background: rgba(220,38,38,0.1); border: 1px solid rgba(220,38,38,0.2); border-radius: 14px; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; animation: icon-glow 3s ease-in-out infinite; }
        @keyframes icon-glow { 0%,100% { box-shadow: 0 0 0 0 rgba(220,38,38,0); } 50% { box-shadow: 0 0 0 6px rgba(220,38,38,0.12); } }
        ...
        .field-icon { position: absolute; left: 14px; top: 50%; translate: 0 -50%; color: rgba(255,255,255,.22); pointer-events: none; transition: color .2s; }
        .field-wrap:focus-within .field-icon { color: rgba(220,38,38,.55); }
        input[type="email"] { width: 100%; padding: 13px 44px; background: var(--in-bg); border: 1px solid var(--in-bdr); border-radius: 11px; color: #fff; font-size: .9rem; font-family: 'Inter', sans-serif; outline: none; transition: border-color .2s, box-shadow .2s, background .2s; -webkit-appearance: none; }
        input[type="email"]:focus { border-color: rgba(220,38,38,.4); background: rgba(220,38,38,.025); box-shadow: 0 0 0 3px rgba(220,38,38,.07); }
...
<div class="page">
    <div class="card">
        ...
        <p class="desc f2">Enter the email address linked to your account and we'll send you a secure reset link.</p>
        ...
        <form method="POST" action="{{ route('portal.forgot-password.send') }}" id="resetForm" novalidate>
            ...
            <div class="field f3">
                ...
                <div class="field-wrap">
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" placeholder="you@example.com" autocomplete="email" autofocus>
                </div>
            </div>
        ...
        <a href="{{ route('portal.login') }}" class="back f5">
