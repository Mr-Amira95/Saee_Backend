...
class OrderController extends Controller
{
    ...
    public function destroy(Order $order): RedirectResponse
    {
        ...
        abort_if($order->client_profile_id !== $profile->id, 403);
        abort_if($order->status !== 'pending', 403, __('Only pending orders can be deleted.'));
        ...
        return redirect()->route('client.orders.index')
            ->with('success', __('Order #:number deleted.', ['number' => $order->order_number]));
    }
    ...
    public function storeConfirmed(Request $request): RedirectResponse
    {
        ...
        if (empty($rows)) {
            return redirect()->route('client.orders.import')->with('error', __('No order data found. Please re-upload your file.'));
        }
